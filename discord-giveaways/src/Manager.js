...
class GiveawaysManager extends EventEmitter {
    ...
    generateMainEmbed(giveaway, lastChanceEnabled) {
        ...
        embed
            ...
            .setDescription(
                `${giveaway.winnerCount} ${winnerText} | Ends at ${new Date(giveaway.endAt).toLocaleString("en-US", {timeZone: "US/Central"}).toString()} (CST)`
            )
        return embed;
    }
    ...
    generateEndEmbed(giveaway, winners) {
		if(giveaway.winnerCount == 1) {
			var winnersText = "winner is";
		} else {
			var winersText = "winners are";
		}
        ...
        const descriptionString = (formattedWinners) => { 
                ...
                return `The ${winnersText} ${formattedWinners}` +
                '\n' +
				giveaway.message.url +
				`
				Giveaway ID ${giveaway.message.id}`
